...
//      configMaskPatterns(channel);
      ...
      LX.LXPatternFactory<Mask_EyePattern> eyePatternFactory = (lx2, ch, l) -> new Mask_EyePattern(lx2, model);
      lx.registerPatternFactory(Mask_EyePattern.class, eyePatternFactory);
      Mask_EyePattern eyePattern = quickBuild(eyePatternFactory).initModulators();
  ...
  private void makeMasks(LXChannel channel) {
    List<LXFixture> whiskers = Arrays.asList(model.getWhiskerFixtures());

    // Any non-standard LX constructors need their own factory registered

    LX.LXPatternFactory<Mask_AllWhite> allWhiteFactory = (lx2, ch, l) -> new Mask_AllWhite(lx2, model.getWhiskerFixtures());
    ...
    LX.LXPatternFactory<Mask_PerlinLineTranslator> perlinLineTranslatorFactory =
            (lx2, ch, l) -> new Mask_PerlinLineTranslator(lx, p, whiskers);
    ...
    LX.LXPatternFactory<Mask_RandomFixtureSelector> rfsFactory =
            (lx2, ch, l) -> new Mask_RandomFixtureSelector(lx2, whiskers);
    ...
    LX.LXPatternFactory<Mask_FixtureDottedLine> fdlFactory =
            (lx2, ch, l) -> new Mask_FixtureDottedLine(lx2, whiskers);
